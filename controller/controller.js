            ...
            for (const item of looper) { 
               ...
               results.forEach( async (r) => { 
                ...
                const querysql = `SELECT  '${r.kdcab}' as kdcab, '${r.kdtk}' as kdtk, '${r.nama}' as nama_toko,
                    ...
                    SUM(IF(RTYPE='J',GROSS - ifnull(PPN,gross*ppn_rate/(1+ppn_rate)),0)) AS TJUAL,
                    SUM(IF(RTYPE='D',GROSS - ifnull(PPN,gross*ppn_rate/(1+ppn_rate)),0)) AS TRET,
                    ...
                    if(rv === "G" || rv === "Gagal" ){
                        ...
                        for(var i = parseInt(start); i<=end; i++){
                            ...
                            qjoin.push(`SELECT kode_toko AS kdtk,DATE('${tgl}') tanggal,
                                    ...
                                    SUM(IF(kode='02' AND subkode NOT IN ('04'),amount_cr+fee_dpp_cr+fee_cr_ppn,0)) +
                                    SUM(IF(kode='11' AND subkode NOT IN ('999','X'),ref_1,0))  Ppn,
